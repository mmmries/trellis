...
This is Phase 1 of #173's five-phase plan. Phase 4 (turning this table into a
real test matrix) and Phase 5 (the encoded-key contract,
#103/#107/#110/#125/#163/#171) remain out of scope for this document.
...
a completeness failure. The third has since been fixed; it is kept here,
...
2. **`ReverseTrigger::WholeKeyspace`'s (via `from_side_keys`) `NULL`-exclusion
   is argued in a doc comment, not asserted in a test.** Nothing breaks today if it's wrong (a
