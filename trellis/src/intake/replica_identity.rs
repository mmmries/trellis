...
//!
//! The two functions are split so the rejection can be tested today:
//! [`needs_old_image`] is the predicate over a [`TransformDef`], but always
//! returns `false` for now because [`KeySpace`] has only [`KeySpace::OneToOne`]
//! and every such transform is a pure function of the *current* row.
//! [`require_replica_identity_full`] takes a plain `bool`, so its test can
//! exercise the `true` side without an aggregate `KeySpace` variant existing
//! yet. When one lands, [`needs_old_image`] grows a `match` arm and the
//! chained call site `require_replica_identity_full(src, needs_old_image(def))`
//! picks up enforcement unchanged.
