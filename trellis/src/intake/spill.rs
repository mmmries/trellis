...
impl TxnBuffer {
    ...
    /// first, then the in-memory tail, both stamped with `end_lsn`/`changed_at`
    /// — with no watermark advance or notify. Issue #268 X4 (intake
    /// group-commit): several source transactions' buffers can each
    /// [`Self::append_only`] into the *same* open `txn` in turn, each
    ...
    /// [`super::advance_watermark_and_notify`] call — once, with the
    /// group's *last* transaction's `end_lsn` — needs to follow, after every
    /// buffer in the group has appended.
