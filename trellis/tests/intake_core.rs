...
/// Issue #268 X4 ("intake group-commit"): three separate single-row source
/// transactions, all committed before the consumer ever starts streaming
/// (so intake decodes their three `Commit`s back to back, well inside a
/// generous `max_delay`), with `group_commit` configured at a `max_rows`
/// none of them individually reach. All three must still land, in full and
/// exactly once — grouping must never drop, duplicate, or reorder a row —
/// and the watermark must still advance normally. (A `pg_stat_database.xact_commit`
/// assertion was tried here and dropped: it's too noisy to assert
/// reliably inside a fast integration test — `ProducerSession::connect`'s
/// own setup queries, the per-relation catalog lookups `handle_xlog_data`
/// makes on a cache miss, and the polling loop below all add their own
/// uncounted autocommit commits to the same database-wide counter. The
/// commit-count reduction this feature is *for* is measured empirically by
/// the benchmark harness instead — see issue #268's X4 writeup.)
#[tokio::test]
async fn group_commit_batches_several_source_transactions_into_fewer_ring_commits() {
    ...
    // transaction" convention elsewhere — see `benchmark/src/streaming/load.rs`),
    // all committed before the consumer connects, so intake sees three
    // `Commit`s in immediate succession once it starts streaming.
    for id in 1..=3i64 {
        setup
            .execute(
