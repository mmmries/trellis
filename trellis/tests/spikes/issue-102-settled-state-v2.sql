...
begin
  -- The watermark can never overshoot what has actually been written: a real
  -- confirmed_lsn only ever advances to an LSN intake has seen and staged.
  -- Without the clamp the model can reach `confirmed >= lsn` while the change
  -- is still unstaged, which no replication stream can produce -- and which
  -- makes the barrier look unsound when it is the harness that is wrong.
  update eng set confirmed = least(confirmed + k, (select coalesce(max(lsn),0) from chg));
...
begin
  ...
  -- The generation must be bumped for every parent the unit's RAW changes
  -- touched, not just for the folded endpoints: folding ins(p1) + repoint(p1->p2)
  -- erases p1 from both images, and a reverse holding a stale enumeration for p1
  -- would then pass its generation check. The fold loses intermediate join-key
  -- values; the validity signal must be taken before it does.
  for f in select o->>'post' as op, n->>'post' as np from chg
            where tbl='pt' and seg=s and bucket=b and not applied
  loop
    perform touch_gen(f.op::bigint);
    perform touch_gen(f.np::bigint);
  end loop;
...
-- Harness self-check: the watermark must never claim to cover an unstaged
-- change. A violation means the model, not the design, is broken -- it is
-- counted rather than raised so a campaign reports it instead of hiding it.
create function assert_invariants() returns void language plpgsql as $$
begin
  if exists (select 1 from chg where seg is null and lsn <= (select confirmed from eng)) then
    perform bump('HARNESS_VIOLATION_unstaged_below_watermark');
  end if;
end $$;
