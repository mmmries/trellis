...
//! in) — the same bridge `engine::intake::publication`'s `fence_snapshot`
...
/// pointer — a plain read, no lock (see `engine::staging::append`).
...
/// (`engine::staging::converge::converged_through`) would otherwise pick a
