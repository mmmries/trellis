...
use engine::staging::{StagingError, TRUNCATE_SENTINEL_KEY, seal};
use testkit::TestCluster;
use tokio_postgres::{Client, NoTls};

/// Connects directly to `dsn` (bypassing `engine::Pool`), for tests that
/// need a plain `tokio_postgres::Client`/`Transaction` — the type
/// `engine::staging::seal`'s functions take, which the pooled
