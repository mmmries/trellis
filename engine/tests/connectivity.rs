...
use engine::{Config, Error, migrate};
...
async fn migrate_up_is_idempotent() {
    ...
    assert_eq!(
        ...
        "expected exactly V1 through V14 to be applied"
    );
...
async fn unreachable_dsn_surfaces_a_typed_error_without_panicking() {
    ...
    .expect("valid schema");
    let pool = engine::Pool::new(&config).expect("pool builds lazily; DSN is well-formed");
