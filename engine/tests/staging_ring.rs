...
use engine::staging::{CdcOp, StagedChange, StagingError, append};
...
/// Connects directly to `dsn` (bypassing `engine::Pool` and its
...
async fn change_id_records_intra_transaction_append_order() {
    ...
    // buffered changes (see `engine::intake::stage_and_advance`).
...
async fn a_session_with_synchronous_commit_off_is_refused() {
    ...
    let err = engine::staging::ProducerSession::connect(&dsn, DEFAULT_SCHEMA)
        .await
...
async fn a_second_producer_cannot_acquire_the_singleton_while_the_first_holds_it() {
    ...
    let first = engine::staging::ProducerSession::connect(db.dsn(), DEFAULT_SCHEMA)
        .await
        .expect("first producer session should be granted the singleton");

    let second = engine::staging::ProducerSession::connect(db.dsn(), DEFAULT_SCHEMA).await;
    match second {
    ...
    for _ in 0..50 {
        if engine::staging::ProducerSession::connect(db.dsn(), DEFAULT_SCHEMA)
            .await
            .is_ok()
        {
            acquired = true;
