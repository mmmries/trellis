...
use engine::defs::ast::{Expr, FieldDef, KeySpace, Predicate, TransformDef, ValueType};
use engine::defs::eval::{RegexCache, Row, Value, evaluate};
...
async fn pg_query_unary(pool: &engine::Pool, sql: &str, arg: &str) -> String {
...
async fn pg_query_binary(pool: &engine::Pool, sql: &str, a: &str, b: &str) -> String {
    let client = pool.get().await.expect("connection");
    let row = client
