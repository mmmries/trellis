...
use engine::defs::{create_relationship, install_definition, render_relationship_select_sql};
...
async fn drain_to_quiescence(pool: &engine::Pool, client: &mut Client) {
    for _ in 0..16 {
        ...
        while apply::drain_once(pool, seg, "nullability_test", 1, "trellis_nullability_test")
            .await
            .expect("drain_once")
            .is_some()
        {}
