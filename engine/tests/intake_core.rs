...
//! (`engine::intake::stage_and_advance`) rather than through a live stream:
//!
...
use engine::intake::{self, IntakeError, replica_identity};
use engine::staging::{CdcOp, StagedChange, TRUNCATE_SENTINEL_KEY};
use testkit::TestCluster;
use tokio_postgres::types::PgLsn;
use tokio_postgres::{Client, NoTls};

/// Connects directly to `dsn` (bypassing `engine::Pool`) and pins
/// `search_path`, matching `engine/tests/staging_ring.rs`'s helper of the
...
async fn notify_is_only_delivered_after_staged_rows_are_visible() {
    ...
    listener
        .batch_execute(&format!(
            "set search_path to {DEFAULT_SCHEMA}, public; listen wake"
        ))
        .await
        .expect("listen");
