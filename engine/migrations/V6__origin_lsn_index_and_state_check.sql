...
-- need statistics — see `engine::staging::seal`'s ANALYZE-on-seal, since
...
-- the convergence predicate (`engine::staging::converge::converged_through`,
-- docs/staging-and-claiming/07-convergence-and-await.md) is defined in terms
-- of *every* state in `engine::staging::state::SegmentState`'s graph:
